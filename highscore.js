...
function getHighScoreTable() {
    ...
    for (var i = 0; i < 10; i++) {
		...
		var sr_record = new ScoreRecord(record[0], record[1]); // to make sure its right, though not required in assignment : var sr_record = new ScoreRecord(record[0], parseInt(record[1]));
		table.push(sr_record);//in js, push() can make the element at the last of the array
    }
...
function addHighScore(record, node) {
    ...
    // Set the attributes and create the text
	//should be in the same row of the line 61-63 object
...
function showHighScoreTable(table) {
    // Show the table
	//console.log(table);//this is for debugging
...
//
//normally, cookie is separated by ';'
//
// Set a cookie
//
